...
export async function POST() {
  ...
  if (!ghlConfigured()) {
    return NextResponse.json({ error: "GoHighLevel not configured. Set GHL_API_KEY and GHL_LOCATION_ID in your environment." }, { status: 400 });
  }
  ...
  try {
    ...
    // Default pipeline = the main "Sales" one (else first).
    ...
    // 3) Opportunities -> deals (exact pipeline + stage) -------------------
    ...
    // 4) Remove the old seeded pipelines/stages (now that deals are remapped).
    let cleaned = false;
    if (stageIdByGhl.size > 0) {
      try {
        ...
        cleaned = true;
      } catch { /* leave seeded rows if cleanup is blocked; board still works */ }
    }
    ...
    return NextResponse.json({
      ...
      ghl_pipelines: ghlPipelines.map((p) => `${p.name} (${(p.stages ?? []).length} stages)`),
    });
  } catch (e: any) {
